...
class TextCleaner:
    ...
    def expand_contractions(self, text: str) -> str:
        ...
        for contraction, expansion in contractions_dict.items():
            text = re.sub(
                rf"\b{re.escape(contraction)}\b", expansion, text, flags=re.IGNORECASE
            )
    ...
    def clean_text(
        ...
        try:
            ...
            logger.info(
                ...
                reduction_percent=round((1 - final_length / original_length) * 100, 2)
...
class DataTransformer:
    ...
    def transform_extraction_result(
        self, extraction_result: Dict[str, Any], request_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Clean the text
            cleaned_text = self.cleaner.clean_text(raw_text, request_id=request_id)
            ...
            if len(normalized_text) > self.settings.processing.max_text_length:
                ...
                normalized_text = normalized_text[
                    : self.settings.processing.max_text_length
                ]
                logger.warning(
                    "Text truncated to max length",
                    request_id=request_id,
                    length=len(normalized_text),
                )
