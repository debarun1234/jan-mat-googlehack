...
class CloudSettings(BaseSettings):
    ...
    bigquery_table_grievances: str = Field(
        default="citizen_grievances", alias="BIGQUERY_TABLE_GRIEVANCES"
    )
    bigquery_table_audit: str = Field(
        default="pipeline_audit", alias="BIGQUERY_TABLE_AUDIT"
    )
    pubsub_topic_submissions: str = Field(
        default="grievances", alias="PUBSUB_TOPIC_SUBMISSIONS"
    )
    pubsub_topic_dlq: str = Field(default="grievances-dlq", alias="PUBSUB_TOPIC_DLQ")
    pubsub_subscription: str = Field(
        default="grievances-sub", alias="PUBSUB_SUBSCRIPTION"
    )
...
class APISettings(BaseSettings):
    ...
    api_description: str = Field(
        default="Production ETL for multimodal citizen input", alias="API_DESCRIPTION"
    )
...
class FileSettings(BaseSettings):
    ...
    allowed_audio_types: List[str] = Field(
        default=["audio/mpeg", "audio/wav", "audio/ogg"], alias="ALLOWED_AUDIO_TYPES"
    )
    allowed_pdf_types: List[str] = Field(
        default=["application/pdf"], alias="ALLOWED_PDF_TYPES"
    )
    temp_upload_dir: str = Field(default="/tmp/uploads", alias="TEMP_UPLOAD_DIR")

    @field_validator(
        "allowed_image_types", "allowed_audio_types", "allowed_pdf_types", mode="before"
    )
    @classmethod
...
class DatabaseSettings(BaseSettings):
    ...
    bigquery_credentials_path: Optional[str] = Field(
        default=None, alias="BIGQUERY_CREDENTIALS_PATH"
    )
...
def validate_settings() -> bool:
    ...
    try:
        ...
        if settings.environment == "production":
            if (
                not settings.cloud.gcp_project_id
                or settings.cloud.gcp_project_id == "your-project-id"
            ):
                raise ValueError("GCP_PROJECT_ID not configured for production")
            if (
                not settings.cloud.gcs_bucket
                or settings.cloud.gcs_bucket == "your-bucket"
            ):
