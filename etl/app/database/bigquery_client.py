...
class BigQueryClient:
    ...
    def _ensure_tables_exist(self):
        ...
        for table_name, schema in tables_to_create:
            table_id = (
                f"{self.settings.cloud.gcp_project_id}.{self.dataset_id}.{table_name}"
            )
            ...
            try:
                ...
                if table_name == self.table_grievances:
                    table.time_partitioning = bigquery.TimePartitioning(
                        type_=bigquery.TimePartitioningType.DAY, field="submitted_at"
                    )
    ...
    def _validate_row_schema(
        self, row: Dict[str, Any], schema: List[SchemaField]
    ) -> bool:
    ...
    def insert_rows(
        ...
        try:
            ...
            if fail_on_duplicate and table_name == self.table_grievances:
                submission_ids = [row.get("submission_id") for row in valid_rows]
                    ...
                    error_details.append(
                        {
                            ...
                            "message": error.get("errors", [{}])[0].get(
    ...
    def query_deduplication_check(self, submission_id: str) -> bool:
