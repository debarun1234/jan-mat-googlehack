...
Remember: ONLY return JSON, no other text."""
    ...
    @retry.Retry(deadline=self.settings.ai.ai_timeout_seconds)
    def infer(
        self,
        text: str,
        ...
        try:
            ...
            response = asyncio.run(self._infer_async(
                system_prompt,
                user_prompt,
                timeout
            ))
            ...
            logger.info(
                f"Gemini inference completed",
                request_id=request_id,
                duration_ms=round(duration_ms, 2),
                tokens_in=response.usage.prompt_token_count if hasattr(response, 'usage') else None,
                tokens_out=response.usage.completion_token_count if hasattr(response, 'usage') else None
            )
