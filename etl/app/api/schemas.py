...
class UploadRequestBase(BaseModel):
    ...
    request_id: Optional[str] = Field(
        None, description="Unique request identifier (auto-generated if not provided)"
    )
    input_type: InputTypeEnum = Field(..., description="Type of input being submitted")
    language: Optional[str] = Field(
        default="en", description="Language code (ISO 639-1)"
    )
    latitude: Optional[float] = Field(
        None, description="Geographic latitude", ge=-90, le=90
    )
    longitude: Optional[float] = Field(
        None, description="Geographic longitude", ge=-180, le=180
    )
...
class UploadResponse(BaseModel):
    ...
    gcs_uri: str = Field(
        ..., description="Google Cloud Storage location of uploaded file"
    )
    estimated_processing_time_seconds: int = Field(
        default=120, description="Estimated time to complete processing"
    )
